...
class VectorRerankService:
    ...
    def __init__(self):
        ...
        self.api_url = f"{config.dashscope_biz_space_api_base}/services/rerank/text-rerank/text-rerank"
        logger.info(f"向量重排服务初始化完成, 模型: {self.model}, API: {self.api_url[:11]}...{self.api_url[-11:]}")
    ...
    def rerank(
        ...
        try:
            ...
            logger.debug(f"Rerank 请求体: {json.dumps(request_body)[:25]}...{json.dumps(request_body)[-25:]}")
            ...
            if response.status_code != HTTPStatus.OK:
                ...
                raise RuntimeError(
                    f"Rerank API 调用失败: {response.status_code} - {error_msg}"
                )
...
if __name__ == '__main__':
    vector_rerank_service.rerank("什么是文本排序模型", [
