...
提供异步 PostgreSQL 连接管理，使用 SQLAlchemy AsyncEngine + asyncpg 驱动。
仅用于 ORM 场景（对话历史持久化），LangGraph 官方的 checkpointer/store 使用 psycopg3 连接。
"""
...
class PostgresClientManager:
    ...
    async def connect(self) -> AsyncEngine:
        ...
        if self._engine is not None:
            logger.debug("PostgreSQL 已连接，跳过重复 connect")
            return self._engine

        try:
            db_url = config.postgres_async_url

            logger.info(
                f"正在连接到 PostgreSQL: {config.postgres_host}:{config.postgres_port}/{config.postgres_db}"
            )
...
# 全局单例
postgres_manager = PostgresClientManager()
